...
class UploadService:
    ...
    def accept_upload(self, match_id: UUID, team_id: UUID, filename: str, content: bytes) -> dict:
        ...
        storage_path = f"{match_id}/{team_id}/{uuid4()}-{filename}"
        self.db.storage.from_(self.bucket).upload(
            storage_path, content, {"content-type": "image/*"}
        )
        return self.screenshot_repo.create(match_id, team_id, storage_path, content_hash)

    def run_ocr_pipeline(self, screenshot_id: UUID, team_id: UUID) -> None:
        ...
        try:
            ...
            roster = self.player_repo.list_for_team(team_id)

            payload: OcrReviewPayload = self.ocr_service.process_screenshot(
                image_bytes, screenshot_id, roster
            )
